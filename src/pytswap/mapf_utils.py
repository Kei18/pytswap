...
def get_random_scenario(
    grid: Grid, N: int, seed: int = 0, rng=None
) -> tuple[Config, Config]:
    if rng is None:
    ...
    starts = deepcopy(candidates[:N])
    rng.shuffle(candidates)
    goals = deepcopy(candidates[:N])
    return starts, goals
...
def validate_amapf_solution(
    ...
    # starts
    assert all([u == v for (u, v) in zip(starts, solution[0])]), (
        "invalid solution, check starts"
    )

    # goals
    assert set(goals) == set(solution[-1]), "invalid solution, check goals"
...
def is_valid_amapf_solution(
    ...
    try:
        validate_amapf_solution(grid, starts, goals, solution)
        return True
